...
let dollar_to_euro (Dollar d) = Euro (0.85 *. d)

let euro_to_dollar (Euro e) = Dollar (1.17 *. e)
...
type currency = 
  Yen of float | Pound of float | Krona of float | Franc of float


let to_pound = function
  | Yen x -> Pound (0.0051 *. x)
  | Pound x -> Pound x
  | Krona x -> Pound (0.079 *. x)
  | Franc x -> Pound (0.93 *. x)
...
type intbool_list =
  | Tail
  | IntEl of int * intbool_list
  | BoolEl of bool * intbool_list


let test = IntEl (5, BoolEl (true, BoolEl (false, IntEl (7, Tail))))

(*----------------------------------------------------------------------------*
  Funkcija `intbool_length ib_list` vrne dolžino `ib_list`.
[*----------------------------------------------------------------------------*)

let rec intbool_length = function
  | Tail -> 0
  | IntEl (_, ibs) -> 1 + intbool_length ibs
  | BoolEl (_, ibs) -> 1 + intbool_length ibs
...
let rec intbool_map f_i f_b = function
  | Tail -> Tail
  | IntEl (i, ibs) -> IntEl (f_i i, intbool_map f_i f_b ibs)
  | BoolEl (b, ibs) -> BoolEl (f_b b, intbool_map f_i f_b ibs)
...
let intbool_reverse ibs =
  let rec aux acc = function
    | Tail -> acc
    | IntEl (i, rest) -> aux (IntEl (i, acc)) rest
    | BoolEl (b, rest) -> aux (BoolEl (b, acc)) rest
  in
  aux Tail ibs
...
let rec intbool_separate ibs =
  ...
  | BoolEl (b, rest) -> aux iacc (b :: bacc) rest
  in
  aux [] [] (intbool_reverse ibs)
...
let professor  = { name = "Charles" ; status = Employed (Fire, Teacher) }
...
let count_magic magicians = 
  let rec aux acc = function
  | [] -> acc
  | mag :: mags  -> 
    match mag.status with
      | Newbie -> aux acc mags
      | Student (magic, _) -> aux (update acc magic) mags
      | Employed (magic, _) -> aux (update acc magic) mags
  in
  aux {fire=0;frost=0;arcane=0} magicians
...
let find_candidate magic specialisation wizard_list = 
  let rec aux = function
    ...
    | mag :: mags -> 
      match mag.status with 
        | Newbie | Employed (_, _) -> aux mags
        | Student (st_magic, years) ->
          match specialisation with
            | Historian when years >= 3 -> Some mag.name
            | Researcher when years >= 4 -> Some mag.name 
            | Teacher when years >= 5 -> Some mag.name
            | _ -> aux mags
  in
  aux wizard_list 
