...
def najdaljse_narascajoce_podazporedje(sez):
    ...
    for i in range(n - 1, -1, -1):
        ...
        for j in range(i + 1, n):
            if sez[j] >= sez[i] and len(rezultati[j]) + 1 > len(rezultati[i]):
                rezultati[i] = [sez[i]] + rezultati[j]
        if len(rezultati[i]) > len(najdaljsi):
            najdaljsi = rezultati[i]
    return najdaljsi
...
def vsa_najdaljsa(sez):
    ...
    for i in range(n - 1, -1, -1):
        trenutni = [sez[i]]
        for j in range(i + 1, n):
            if sez[j] >= sez[i] and len(rezultati[j]) + 1 > len(trenutni):
                trenutni = [sez[i]] + rezultati[j]
        rezultati[i] = trenutni

        if len(trenutni) > len(najdaljsi[0]):
            najdaljsi = [trenutni]
        elif len(trenutni) == len(najdaljsi[0]):
            najdaljsi.append(trenutni)  
    return najdaljsi
